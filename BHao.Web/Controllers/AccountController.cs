...
namespace BHao.Web.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                var korisnik = await UserManager.FindByNameAsync(model.Email);
                if (korisnik != null)
                {
                    if (korisnik.IsDeleted)
                    {
                        ModelState.AddModelError("", String.Format("Vaš korisnički nalog je onemogućen."));
                    }
                }
            }
        ...
        public ActionResult Register()
        {
            ...
            {
                GradRepository repo = new GradRepository(context);

                gradovi = repo.GetAll().ToList();
            }

            ViewBag.Gradovi = new SelectList(gradovi, "Id", "Naziv");
        ...
        private void AddErrors(IdentityResult result)
        {
            foreach (var error in result.Errors)
            {
                ...
                {
                    var ime = "Korisničko ime već postoji.";
                    ModelState.AddModelError("", ime);
                }
                else if (error.StartsWith("Email"))
                {
                    var email = "Korisnik sa email adresom već postoji.";
